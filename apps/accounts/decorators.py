...
def customer_is_not_banned(function=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url='../accounts/login'):
    '''
    Decorator for views that checks that the logged in user is a teacher,
    redirects to the log-in page if necessary.
    '''
    ...
    actual_decorator = user_passes_test(
        lambda u: u.is_active and u.is_customer and u.is_not_banned,
        login_url=login_url,
        redirect_field_name=redirect_field_name
    )
    if function:
